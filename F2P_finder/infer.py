...
def _default_test_id(language: str, file_path: str, hunk_index: int) -> str:
    ...
    if language == "python":
        return f"{file_path}::test_hunk_{hunk_index + 1}"
    if language == "go":
        return f"{file_path}/TestHunk{hunk_index + 1}"
    if language == "rust":
        return f"{Path(file_path).stem}::test_hunk_{hunk_index + 1}"
    return f"{Path(file_path).stem}.test_hunk_{hunk_index + 1}"
...
def _collect_test_candidates(test_patch: str, language: str) -> Dict[str, TestCandidate]:
    ...
    for file_patch in files:
        ...
        for idx, hunk in enumerate(file_patch.hunks):
            ...
            for line in added:
                ids = extract_test_ids(lang, file_patch.path, line)
                added_ids |= ids
                test_ids |= ids
            for line in removed:
                ids = extract_test_ids(lang, file_patch.path, line)
            ...
            if not test_ids:
                test_ids.add(_default_test_id(lang, file_patch.path, idx))
            ...
            for test_id in test_ids:
                ...
                if test_id in added_ids and test_id not in removed_ids:
...
def infer_from_patches(
    ...
    for candidate in candidates.values():
        tid = canonicalize_test_name(lang, candidate.test_id)
